(** Low-level bindings to QuickJS's libregexp - Regular Expression Engine
    ...
    ECMAScript-compatible regex engine with Unicode support. These are raw C
...
(** Execute a compiled regex against input string. [deadline] is an optional
    ...
    match, 0 = no match, -1 = memory error, -2 = timeout, -3 = invalid bytecode
*)
...
let get_alloc_count bc_buf =
  Quickjs_bindings.C.Functions.lre_get_alloc_count bc_buf

(** Get pointer to the engine-owned group-name records *)
let get_groupnames bc_buf =
  Quickjs_bindings.C.Functions.lre_get_groupnames bc_buf

let get_groupname bc_buf group_index =
  Quickjs_bindings.C.Functions.lre_get_groupname bc_buf group_index

...
(* ===================================================================
   ...
   Returns Some match_result, or None if no match
   =================================================================== *)
...
let a2_t1 () =
  ...
  let re = regexp_compile "(abc)" ~flags:"" in
  let result = RegExp.exec re "abc" in
  assert_match result [| "abc"; "abc" |]

let a2_t2 () =
  (* Multiple capture groups *)
  let re = regexp_compile "(a)(b)(c)" ~flags:"" in
  let result = RegExp.exec re "abc" in
  assert_match result [| "abc"; "a"; "b"; "c" |]

let a2_t3 () =
  (* Nested capture groups *)
  let re = regexp_compile "((a)(b))" ~flags:"" in
  let result = RegExp.exec re "ab" in
  assert_match result [| "ab"; "ab"; "a"; "b" |]

let a2_t4 () =
  (* Optional group that doesn't match is None (undefined in JS),
     distinct from a group that matched the empty string *)
  let re = regexp_compile "a(b)?c" ~flags:"" in
  let result = RegExp.exec re "ac" in
  assert_captures result [| Some "ac"; None |]
...
let a2_t5 () =
  ...
  assert_captures result [| Some "a"; Some "a"; None |]

let a2_t6 () =
  (* Quantified group *)
  let re = regexp_compile "(ab)+" ~flags:"" in
  let result = RegExp.exec re "abab" in
  assert_match result [| "abab"; "ab" |]
(* last match of group *)
...
let a2_t7 () =
  ...
  assert_match result [| "dog"; "dog" |];
  assert_match_index result 9

let a2_t8 () =
  (* Non-capturing group *)
  let re = regexp_compile "(?:a)(b)" ~flags:"" in
  let result = RegExp.exec re "ab" in
  assert_match result [| "ab"; "b" |]
(* only one capture *)
...
let a3_t1 () =
  ...
  assert_match result1 [| "a" |];
  assert_match_index result1 1;
  assert_int (RegExp.last_index re) 2;

  let result2 = RegExp.exec re input in
  assert_match result2 [| "a" |];
  assert_match_index result2 3;
  assert_int (RegExp.last_index re) 4;

  let result3 = RegExp.exec re input in
  assert_match result3 [| "a" |];
  assert_match_index result3 5;
  assert_int (RegExp.last_index re) 6;

  (* No more matches - returns None and resets lastIndex *)
  let result4 = RegExp.exec re input in
  assert_no_match result4;
  assert_int (RegExp.last_index re) 0

let a3_t2 () =
  (* Without global flag - always starts from beginning, and lastIndex
     stays untouched (JavaScript semantics) *)
  let re = regexp_compile "a" ~flags:"" in
  let input = "banana" in
...
let a3_t5 () =
  ...
  let result1 = RegExp.exec re input in
  assert_match result1 [| "1" |];

  let result2 = RegExp.exec re input in
  assert_match result2 [| "22" |];

  let result3 = RegExp.exec re input in
  assert_match result3 [| "333" |];

  let result4 = RegExp.exec re input in
  assert_match result4 [| "4444" |];

  let result5 = RegExp.exec re input in
  assert_no_match result5
...
let a3_t6 () =
  ...
  let result1 = RegExp.exec re input in
  assert_match result1 [| "abc" |];
  assert_match_index result1 0;

  let result2 = RegExp.exec re input in
  assert_match result2 [| "abc" |];
  assert_match_index result2 3;

  let result3 = RegExp.exec re input in
  assert_no_match result3
...
let a4_t1 () =
  ...
  assert_match result [| "hello"; "hello" |];
  assert_group result "name" (Some "hello")

let a4_t2 () =
  (* Multiple named groups *)
  let re = regexp_compile "(?<first>\\w+) (?<last>\\w+)" ~flags:"" in
  let result = RegExp.exec re "John Doe" in
  assert_match result [| "John Doe"; "John"; "Doe" |];
  assert_group result "first" (Some "John");
  assert_group result "last" (Some "Doe")
...
let a4_t3 () =
  ...
  assert_group result "year" (Some "2024");
  assert_group result "month" (Some "07");
  assert_group result "day" (Some "17")

let a4_t4 () =
  (* Non-existent named group returns None *)
  let re = regexp_compile "(?<name>\\w+)" ~flags:"" in
  let result = RegExp.exec re "hello" in
  assert_group result "nonexistent" None

let a4_t5 () =
  (* Named group that doesn't participate in match is present with None *)
  let re = regexp_compile "(?<a>a)|(?<b>b)" ~flags:"" in
  let result = RegExp.exec re "b" in
  assert_group result "b" (Some "b");
  assert_group result "a" None;
  match result with
  | Some m ->
      assert_int (List.length m.RegExp.groups) 2;
      (* named groups are listed in source order *)
      assert_bool (List.map fst m.RegExp.groups = [ "a"; "b" ]) true
...
let a4_t6 () =
  ...
  assert_match result [| "123-test-456"; "123"; "test"; "456" |];
  assert_group result "name" (Some "test")
...
let a4_t7 () =
  ...
  let result = RegExp.exec re input in
  assert_group result "word" (Some "hello");
  let result = RegExp.exec re input in
  assert_group result "word" (Some "world")
...
let a4_t8 () =
  ...
  assert_group result "name" (Some "test");
  match result with
  | Some m -> assert_int (List.length m.RegExp.groups) 1
  | None -> Alcotest.fail "expected a match"
...
let a4_t9 () =
  ...
  assert_match result [| "123"; "123" |];
  match result with
  | Some m -> assert_int (List.length m.RegExp.groups) 0
  | None -> Alcotest.fail "expected a match"
...
let edge_word_boundary () =
  ...
  assert_match result1 [| "word" |];

  let result2 = RegExp.exec re "wording" in
  assert_no_match result2
